...
class CustomDatePicker: UIView {
    ...
    static let instance = CustomDatePicker()
...
//        }
    ...
    func setupView1(min:Date,max:Date){
        ...
        dateView.clipsToBounds = true
        datePicker.minimumDate = min
        datePicker.timeZone = TimeZone.current //eman
      //  datePicker.maximumDate = max
    ...
    func returnSelectedDate(max:Date = Calendar.current.date(byAdding: .year, value: 100, to: Date()) ?? Date(),min:Date = Date(),current:Date,selectedDate: @escaping (_ stringDate:String) -> ()) {
        
        
/*
        if HomeComponent.searchSingelton.dateFrom?.toDate ?? Date() < date
        {*/
          //  datePicker.date = date
            self.yesCompletion = selectedDate
            DispatchQueue.main.async {
                self.setupView(min: min, max: max, current: current)
            }
       // }
        
    }
    //Calendar.current.date(byAdding: .year, value: 100, to: Date()) ??
    func returnSelectedDate1(max:Date = Date(),min:Date = Date(),selectedDate: @escaping (_ stringDate:String) -> ()) {
        
       /* if HomeComponent.searchSingelton.dateFrom!.toDate < date
        {*/
         //   datePicker.date = date
            self.yesCompletion = selectedDate
    print("Log chiheb Return date" , min)
            DispatchQueue.main.async {
                self.setupView1(min: min , max: max)
    ...
    func returnDateOfBirth(max: Date = Date(),min:Date = Calendar.current.date(byAdding: .year, value: -100, to: Date()) ?? Date(),selectedDate: @escaping (_ stringDate:String) -> ()){
