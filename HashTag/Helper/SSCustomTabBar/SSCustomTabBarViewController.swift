        ...
        get {
            if lang == "ar"
            {
                return tabBar.subviews.filter({ $0 is UIControl })
                    .sorted(by: { $0.frame.minX < $1.frame.minX }).reversed()
...
extension SSCustomTabBarViewController {
    ...
    public func addOrUpdateBadgeValueAtIndex(index: Int, value: String) {
    ...
    public func removeBadgeValueAtIndex(index: Int) {
        guard let item = getTabBarItemAtIndex(index) else { return }
        item.badgeValue = nil
    }
