...
//  HashTag
//
//  Created by Trend-HuB on 16/12/1444 AH.
//
...
class CheckUpdate: NSObject {
    ...
    static let appStoreId = "1593043691" // Id Example
...
extension UIViewController {
    ...
    @objc fileprivate func showAppUpdateAlert(version : String, force: Bool, appURL: String, isTestFlight: Bool) {
        ...
        let alertTitle = "New version".localiz()
        let alertMessage = Constants.messages.forceUpdateMsg
        let alertController = UIAlertController(title: alertTitle, message: alertMessage, preferredStyle: .alert)

        let updateButton = UIAlertAction(title: Constants.messages.Update, style: .default) { (action:UIAlertAction) in
