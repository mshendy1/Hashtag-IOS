...
//  zVendo
//
//  Created by Ahmad Ismail on 1/16/19.
...
extension UICollectionViewFlowLayout {
     func getWidthOfCollectionItem(collectionWidth: CGFloat , visibleItemsCount: CGFloat) -> CGFloat {
        var totalSpace: CGFloat
        if #available(iOS 11.0, *) {
            totalSpace = self.sectionInset.left
                + self.sectionInset.right
                + (self.minimumInteritemSpacing * visibleItemsCount)
        } else {
        ...
        return (collectionWidth - totalSpace) / visibleItemsCount
    }
}
