...
public class SheetViewController: UIViewController {
    ...
    /// Allow bouncing under minimum value
    public var allowBounceUnderMinValue = true
    ...
    public init(controller: UIViewController, sizes: [SheetSize] = [.intrinsic], options: SheetOptions? = nil) {
        ...
        if #available(iOS 13.0, *) {
            self.contentViewController.contentBackgroundColor = UIColor.systemBackground.withAlphaComponent(0.8)
        } else {
            self.contentViewController.contentBackgroundColor = UIColor.white.withAlphaComponent(0.8)
        }
    ...
    @objc func panned(_ gesture: UIPanGestureRecognizer) {
        ...
        if newHeight < minHeight {
            offset = !allowBounceUnderMinValue ? 0 : minHeight - newHeight
            newHeight = minHeight
        }
...
extension SheetViewController: UIGestureRecognizerDelegate {
    ...
    public func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if velocity.y < 0 {
            ...
            if height(for: self.orderedSizes.first) >= height(for: self.currentSize) && !allowBounceUnderMinValue { // to prevent sheet controller from bounce if user at minimum value
                return false
            }
            return true
        }
